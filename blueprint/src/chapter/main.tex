...
MeasureTheory.AEStronglyMeasurable.globalMaximalFunction, laverage_le_globalMaximalFunction}
...
\textit{This section is currently under construction, to incoroporate the proof of \Cref{linearised-metric-Carleson}}\\
...
\leanok
\lean{Antichain.local_antichain_density}
...
\begin{lemma}[tree projection estimate]
    ...
    \end{equation}
\end{lemma}
%fixme: It seems like the constant is off, it should be 2^{255a^3} I think.
...
\begin{proof}[Proof of \Cref{tree-projection-estimate}]
    ...
    We apply \Cref{pointwise-tree-estimate} to $e(\fcc(\fu)) f$ to obtain for all $y, x' \in L$
    $$
...
\begin{proof}[Proof of \Cref{boundary-operator-bound}]
    ...
    \end{align}
    By \Cref{boundary-overlap}, there are at most $2^{9a}$ cubes $I$ at each scale with $J \subset B(c(I), D^{s(I)})$.
...
\begin{lemma}[Lipschitz Holder approximation]
    ...
    \leanok
    \lean{support_holderApprox_subset, dist_holderApprox_le, lipschitzWith_holderApprox}
...
% Note: Sébastien Gouëzel commented that $k$ should live in $\mathbb{Z}$ instead of $\mathbb{N}$
Define
$$
...
\begin{lemma}[nontangential-from-simple]\label{nontangential-from-simple}
    \uses{simple-nontangential-operator, tang-unm-op-ball-diff}
...
\begin{lemma}[Weak 1 1]
    ...
    \end{equation}
\end{lemma}
Throughout \Cref{subsec-CZD,subsec-cotlar}, for any measurable bounded function $w: X \to \C$, let $Mw: X \to [0, \infty)$ denote the corresponding Hardy--Littlewood maximal function defined in \Cref{Hardy-Littlewood}.
Apart from \Cref{Hardy-Littlewood}, \Cref{subsec-CZD,subsec-cotlar} have no dependencies in the previous chapters.
...
The set $F_2$ is then estimated with \Cref{weak-1-1} by
\begin{equation}
...
\end{lemma}
\begin{proof}
With \Cref{estimate-x-shift} and the triangle inequality, we estimate for every $x\in X$
\begin{equation}
     T_{*}^r g(x)
...
\end{equation}
We now write using \Cref{tang-unm-op-ball-diff} and the triangle inequality,
...
\begin{lemma}[Lebesgue differentiation]
    ...
    \lean{VitaliFamily.ae_tendsto_average}
...
%fixme: The following is probably part of mathlib (for a second countable topology?)
...
%\begin{lemma}[Vitali covering]
%    \label{Vitali-covering}
%    \leanok
%    Let $B_i$, $i\in I$, be a family of balls with center $x_i$ and radius $r_i$ such that $\sup_{i\in I} r_i < \infty$. Then there exists $J\subset I$ such that
%    \begin{equation}
%        \label{vitali-balls-disjoint}
%        B_j \cap B_{j'} = \emptyset \quad \text{ for } j \ne j', \, j,j' \in J
%    \end{equation}
%    and
%    \begin{equation}
%        \label{vitali-enlarged-balls-covering}
%        \forall i\in I, \exists j \in J, B_i \subset B (x_j, 6r_j) .
%    \end{equation}
%\end{lemma}
%\begin{proof}
%    This is almost part of mathlib (it is proven there for closed balls and constant $5$ instead of $6$).
%\end{proof}
...
We use \Cref{Calderon-Zygmund-decomposition} to prove \Cref{weak-1-1}. For the remainder of this section, let $f:X\to\C$, $r>0$ and $\alpha>0$ as in the lemma.
...
\begin{lemma}
    ...
    For $F$ as defined in \Cref{estimate-Tr-bad-X-minus-Omega}, we have
    \begin{equation}
...
\begin{lemma}[Estimate bad]
    \label{estimate-bad}
    \uses{Calderon-Zygmund-decomposition,estimate-Tr-bad-X-minus-Omega,estimate-F-set}
    We have
    \begin{equation*}
...
\begin{proof}
    ...
    \end{equation}
    It follows from \Cref{estimate-Tr-bad-X-minus-Omega} and the triangle inequality that
    \begin{equation}
        \label{eq-set-dec-3}
...
\begin{proof}[Proof of \Cref{weak-1-1}]
\proves{weak-1-1}
